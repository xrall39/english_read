...
- [x] 添加学习模式
  - [x] 创建FlashCard单词卡片组件（翻转动画、语音朗读）
  - [x] 创建学习页面（/learn）
  - [x] 支持简单模式（认识/不认识）和高级模式（0-5评分）
  - [x] 键盘快捷键支持
- [x] 创建复习算法
  - [x] 实现SM-2间隔重复算法（基于艾宾浩斯遗忘曲线）
  - [x] 创建学习API路由（/api/learn）
  - [x] 数据库支持（next_review, ease_factor, interval_days等字段）
- [x] 实现学习进度统计
  - [x] 创建统计页面（/stats）
  - [x] 创建统计API路由（/api/stats）
  - [x] 掌握程度分布图
  - [x] 学习趋势图表
  - [x] 连续学习天数统计
...
### 2025-12-22 更新
- 集成DeepLX翻译服务
  - 添加DeepLX翻译函数到翻译API路由
  - 实现翻译服务降级策略：本地词典 → DeepLX → 返回未找到
  - 创建环境变量示例文件 (`frontend/.env.local.example`)
  - 更新CLAUDE.md文档，添加翻译服务和环境变量说明

### 2025-12-21 更新
- 实现学习模式和复习算法
  - 创建SM-2间隔重复算法 (`frontend/src/lib/spaced-repetition.ts`)
  - 创建学习API路由 (`/api/learn`)
  - 创建统计API路由 (`/api/stats`)
  - 创建FlashCard单词卡片组件（翻转动画、语音朗读）
  - 创建学习页面 (`/learn`)
  - 创建统计页面 (`/stats`)
  - 支持简单模式和高级模式切换
  - 键盘快捷键支持（空格翻转，数字评分）
- 数据库更新
  - vocabulary表添加间隔重复字段（next_review, ease_factor, interval_days, consecutive_correct）
  - 新增learning_sessions表
  - 添加相关索引优化查询性能
- 导航更新
  - Header添加"学习"和"统计"链接
  - 生词本页面添加"开始学习"和"学习统计"按钮
